...
var CONNECTION_URL_PRO = "mongodb+srv://" + userPassword + "@" + CLUSTER_NAME_PRO + "-uvudw.mongodb.net/";
var CONNECTION_URL_DEV = "mongodb+srv://" + userPassword + "@" + CLUSTER_NAME_DEV + "-uvudw.mongodb.net/";
var CONNECTION_URL_LOG = "mongodb+srv://" + userPassword + "@" + CLUSTER_NAME_LOG + "-uvudw.mongodb.net/";
var CONNECTION_URL_OLD = "mongodb+srv://" + userPassword + "@" + CLUSTER_NAME_OLD + "-uvudw.mongodb.net/";
var CONNECTION_URL_VC = "mongodb+srv://" + userPassword + "@" + CLUSTER_NAME_VC + "-uvudw.mongodb.net/";

var CONN_OPTION_TAIL = '?retryWrites=true&w=majority';
var CONN_OPTION = { useUnifiedTopology: true, useNewUrlParser: true, poolSize: POOL_SIZE };
...
var CONN_LIST_OBJ = {
    ...
    ,'voucherCodes': { connObj: undefined, cluster: CONNECTION_URL_VC, db: 'dbVoucherCodes' }
    ,'voucherCodesDev': { connObj: undefined, cluster: CONNECTION_URL_VC, db: 'dbVCDev' }
...
app.post("/item", (request, response) => {
    ...
    AppUtil.getConnObj( 'item', '', MAPPING_ARRAY, CONN_LIST_OBJ, function( dbConn, dbDef )
    {
...
app.post("/:stage/:item", (request, response) => {
    ...
    AppUtil.getConnObj( stage, item, MAPPING_ARRAY, CONN_LIST_OBJ, function( dbConn, dbDef )
    {        
...
AppUtil.getConnObj = function( stage, item, mappingArr, connListObj, returnFunc )
{
    try
    {
...
AppUtil.processDbColl = function( dbConn, collName, request, response, requestName )
{
    var errMsg = "";
    ...
    try 
    {
        ...
        else errMsg = 'FAILED.  Could Not find matching db connection for ' + requestName;
    }
    catch( ex )
    {
...
AppUtil.processData = function( request, response, collection )
{
    ...
    if ( request.body && request.body.mongoDB )
    {
        ...
        if ( mongoDB.find )
        {
            ...
            collection.find( mongoDB.find ).project( mongoDB.project ).limit( queryLimit ).sort( mongoDB.sort ).toArray( ( error, result ) => {
                AppUtil.processResult( response, error, result, startDateTime, resultJson, mongoDB );                
            });
        }
        ...
        {
            resultJson.NOTE = "insert command is OBSOLETE.  Used insertOne or insertMany instead.";

            collection.insert( mongoDB.insert, (error, result) => {
                AppUtil.processResult( response, error, result, startDateTime, resultJson );
            });
        }
        ...
        {
            resultJson.NOTE = "update command is OBSOLETE.  Used updateOne or updateMany instead.";
            var updateObj = mongoDB.update;

            // NOTE: this 'idenObj' name maybe better suited by 'find'?
            Util.formatIdField( updateObj.idenObj );

            collection.update( updateObj.idenObj, updateObj.updateData, (error, result) => {
                AppUtil.processResult( response, error, result, startDateTime, resultJson );
            });
        }
        else if ( mongoDB.updateOne )
        {
            ...
            collection.updateOne( updateObj.idenObj, updateObj.updateData, (error, result) => {
                AppUtil.processResult( response, error, result, startDateTime, resultJson );
            });
        }
        else if ( mongoDB.updateMany )
        {
            ...
            collection.updateMany( updateObj.idenObj, updateObj.updateData, (error, result) => {
                AppUtil.processResult( response, error, result, startDateTime, resultJson );
            });
        }
        ...
        {
            resultJson.NOTE = "remove command is OBSOLETE.  Used deleteOne or deleteMany instead.";
    ...
    {
        response.send( { 'query': request.query, 'body': request.body, 'note': 'Make sure payload body is JSON, not TEXT.' } );
    }

};


AppUtil.processResult = function( response, error, result, startDateTime, resultJson, mongoDB )
{
    if ( error ) {
